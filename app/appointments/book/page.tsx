...
interface Doctor { _id: string; name: string; slug: string; specialization: string; department: { name: string } }
...
function BookAppointmentContent() {
  ...
  const [form, setForm] = useState({ doctorSlug: preselectedDoctor ?? "", date: "", time: "", reason: "" });
  ...
  useEffect(() => {
    if (!user) { router.push(`/auth/login?redirect=/appointments/book`); return; }
    fetch("/api/doctors").then((r) => r.json()).then((d) => setDoctors(d.doctors ?? []));
  }, [user, router]);

  useEffect(() => {
    if (!form.doctorSlug || !form.date) { setSlots([]); return; }
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/appointments", {
        ...
        body: JSON.stringify({ doctorSlug: form.doctorSlug, dateTime, reason: form.reason }),
      });
  ...
  if (success) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center max-w-md w-full">
        ...
        <p className="text-gray-500 text-sm mb-2">A confirmation email has been sent to you.</p>
        <p className="text-xs text-gray-400 mb-6">Check your inbox for appointment details and pre-visit instructions.</p>
        <div className="flex gap-3 justify-center">
          <button onClick={() => router.push("/portal")} className="px-6 py-2.5 bg-sky-600 text-white rounded-xl text-sm font-semibold hover:bg-sky-700">Go to Portal</button>
          <button onClick={() => { setSuccess(false); setForm({ doctorSlug: "", date: "", time: "", reason: "" }); }} className="px-6 py-2.5 border border-gray-200 text-gray-700 rounded-xl text-sm font-medium hover:bg-gray-50">Book Another</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-1">Book Appointment</h1>
          <p className="text-gray-500 text-sm">Schedule a consultation with our specialists</p>
        </div>
        ...
        <form onSubmit={submit} className="bg-white rounded-2xl border border-gray-100 p-8 space-y-5">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setForm({ ...form, doctorSlug: e.target.value, time: "" })}
          ...
          <div className="bg-sky-50 rounded-xl p-4 flex gap-2.5 text-xs text-sky-800">
            ...
            <span>By booking, you consent to the collection and processing of your appointment data under your existing consent preferences. <a href="/consent/dashboard" className="underline">Manage consents</a></span>
          </div>
