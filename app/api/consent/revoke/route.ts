...
export async function POST(req: NextRequest) {
  try {
    ...
    const found = await applyConsentFlag(referenceId.trim(), { consentRevoked: true });
    if (!found) return NextResponse.json({ error: "No record found for referenceId." }, { status: 404 });
    return NextResponse.json({ message: "onConsentRevoke applied." });
  } catch (err) {
    console.error("[consent/revoke]", err);
