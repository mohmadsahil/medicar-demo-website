...
export async function POST(req: NextRequest) {
  try {
    ...
    const found = await applyConsentFlag(referenceId.trim(), { consentExtended: true });
    if (!found) return NextResponse.json({ error: "No record found for referenceId." }, { status: 404 });
    return NextResponse.json({ message: "onConsentExtension applied." });
  } catch (err) {
    console.error("[consent/extension]", err);
